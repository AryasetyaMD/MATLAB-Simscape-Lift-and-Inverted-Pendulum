...
static void mdlUpdate(SimStruct *S, int_T tid) {
    ...
    real_T isd_act_filtered_old = X[15];
    ...
    // Adaptive Filter isd_act
    real_T wp = 2 * pi * fp_isd_act; // cutoff frequency (rad/s)
    real_T alpha = wp * dt / (1 + wp * dt);
    real_T isd_act_filtered = alpha * isd_act + (1 - alpha) * isd_act_filtered_old;

    // Torque
    real_T Te = Np * (1 - sigma) * Ls * imr_old * isq_act;

    real_T imr_new = imr_old + ((Rr / Lr) * (isd_act_filtered - imr_old) * dt);

    real_T integral_isd_new = integral_isd_old + (isd_ref - isd_act_filtered) * dt;
    real_T integral_isq_new = integral_isq_old + (isq_ref - isq_act) * dt;

    real_T usd = (Kidp * (isd_ref - isd_act_filtered)) + (Kidi * integral_isd_new);
    real_T usq = (Kiqp * (isq_ref - isq_act)) + (Kiqi * integral_isq_new);

    real_T Vcd = -(omega_e * Ls * sigma * isq_act) + Ls * (1 - sigma) * imr_new;
    real_T Vcq = omega_e * Ls * sigma * isd_act_filtered + Ls * (1 - sigma) * omega_e * imr_old;
